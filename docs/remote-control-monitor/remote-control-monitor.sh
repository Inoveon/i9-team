...
# Parâmetros configuráveis via EnvironmentFile (ver systemd unit)
# Defaults aplicados se não vierem do ambiente.
WHATSAPP_ENABLED="${WHATSAPP_ENABLED:-true}"
CLAUDE_ENV_PREFIX="${CLAUDE_ENV_PREFIX:-}"
...
send_whatsapp() {
  ...
  if [ "$WHATSAPP_ENABLED" != "true" ]; then
    log "whatsapp_disabled (WHATSAPP_ENABLED=$WHATSAPP_ENABLED) — skipping"
    return 0
  fi
...
process_orchestrator() {
  ...
  local env_label="${CLAUDE_ENV_PREFIX:-<sem prefixo>}"
  local host_label="$(hostname -s 2>/dev/null || echo '?')"
  local msg
  msg=$(cat <<EOF
$status_icon Remote Control — ação executada

🌎 Ambiente: $env_label ($host_label)
📍 $project / $team
🧭 $agent
...
main() {
  ...
  local start_ts
  start_ts=$(date +%s)
  log "tick start (WHATSAPP_ENABLED=$WHATSAPP_ENABLED, CLAUDE_ENV_PREFIX='${CLAUDE_ENV_PREFIX:-<none>}')"

  # Prefixo opcional de ambiente pro nome do Remote Control
  local prefix_part=""
  [ -n "$CLAUDE_ENV_PREFIX" ] && prefix_part="${CLAUDE_ENV_PREFIX}-"

  # Itera orquestradores: projeto, team, agente, sessão, nome_remote_base
  local count=0
  while IFS=$'\t' read -r project team agent session rc_name_base; do
    [ -z "$project" ] && continue
    count=$((count + 1))
    local rc_name="${prefix_part}${rc_name_base}"
